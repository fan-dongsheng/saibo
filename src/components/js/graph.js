...
function add_nodes() {
    ...
    //         if (img != undefined) {
    //           console.log(d.image)
    //             return "http://localhost:8000/path/?name=" + d.image
    //         } else {
    //             return null;
    //         }
    //     })
...
function addToolTip(d) {
    ...
    if (d.image != undefined) {
        htmlStr = htmlStr + "<img src=\"../../assets/?name=" + d.image + "\" height=\"100\" width=\"100\" />";
        // console.log(htmlStr)
    }
...
function adddialog(d) {
  var htmlStr;
  htmlStr = '这是详细信息对话框';
  var  tooltip= d3.select("#showInfo").append("div")
    .attr("class", "tooltip") //用于css设置类样式
    .attr("opacity", 0.0)
    .attr("id", "dialog");
...
function addToolTipCir(d) {
    ...
    if (d.name == "✂") {
        htmlStr = "裁剪当前节点与关系<br/>";
    }
    if (d.name == "✠") {
        htmlStr = "拓展当前节点与关系<br/>";
    }
    if (d.name == "◎") {
        htmlStr = "释放所有锁定的节点<br/>";
    }
    if (d.name == "오") {
        htmlStr = "锁定所有节点<br/>";
    }
...
function drawCirclePartition(d) {
  ...
  function EdgeIsInArray(arr, value) {
    ...
    function myExtendNode() {
      axios.get('api' + '/kgR1?node=' + d.name)
        .then(res=>{
          ...
          for (let i=0;i<comData.nodes.length;i++) {
            if (!isInArray(nodes, comData.nodes[i].name)){
              arrNodes.push(comData.nodes[i])
            }
          }
          nodes = nodes.concat(arrNodes)
          // console.log(nodes)
          for (let i=0;i<comData.links.length;i++) {
            let obj2 = comData.links[i]
            if (!EdgeIsInArray(edges, comData.links[i])){
              obj2.source = d.index
              nodes.forEach((val,i)=>{
                if (obj2.target_name===val.name){
                  obj2.target = i
                }
              })
              arrEdges.push(obj2)
          ...
          edges = edges.concat(arrEdges)
          edges.forEach((val,i)=>{
            val.index = i
          })
          // console.log(edges[6].source.index)
          // for (let i=0;i<edges.length;i++) {
          //   // console.log(edges[i])
          //   edges[i].source.index = i
          //   edges[i].target.index = i+1
          // }
...
export function load(datakg,wide) {
  window.width = wide, window.height = 600, window.color = d3.scale.category20();
  window.svg = d3.select("#kgp")
    .append("svg")
    .attr("id", "svgGraph")
    .attr("width", width)
    .attr("height", height)
  ...
  graph(datakg)
  dataCirclePartition = {"children": [{"name": "☿"}, {"name": "✂"}, {"name": "✠"}, {"name": "◎"}, {"name": "오"}]}
}
