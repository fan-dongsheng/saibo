        ...
        <el-card class="list">
          ...
          <div class="list-cont" v-for="(item,index) in listMark" :key="index">
            <el-button
              ...
              round
              @click="selectText(item.color,item.label)"
...
export default {
  data () {
    return {
      ...
      entDataPath: this.$route.query.dataPath || this.$route.query.dirpath,
  ...
  methods: {
    //跳转新页面展示表格
    prview (entDataPath) {
      console.log("entDataPath:", entDataPath)
    ...
    downRelation (file) {
      ...
      this.$router.push({ path: '/prview', query: { file_path: file } })
        ...
        try {
          ...
          if (this.$route.query.dirpath) {
            const { data } = await this.$ajax({
              url: '/hehe/ner',
              params: {
                modelpath: this.$route.query.dirpath,
                dataset: this.$route.query.dirpath,
                // dirpath: this.$route.query.dirpath, 
                filelist: this.$route.query.filenames
              }
            })
